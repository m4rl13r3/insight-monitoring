...
function hourly_bootstrap_context() {
    ...
    $loadedConfig = is_file($configPath) ? include $configPath : [];
    $allowedOriginsRaw = is_array($loadedConfig) ? trim((string)($loadedConfig['allowed_origins'] ?? '')) : '';
    ...
    $config = null;
    if (
        is_array($loadedConfig) &&
        !empty($loadedConfig['servername']) &&
        !empty($loadedConfig['username']) &&
        array_key_exists('password', $loadedConfig) &&
        !empty($loadedConfig['dbname'])
    ) {
        $config = $loadedConfig;
    }
