...
from python_monitoring.notifications import DEFAULT_TEMPLATES, dispatch_event, render_templates, send_channel
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Notifications Insight")
    parser.add_argument("action", choices=["send", "dispatch", "render"])
    ...
    try:
        ...
        if args.action == "send":
            ...
            try:
                result = dispatch_event(database, cfg, event_key, context, force=bool(args.force))
            finally:
                database.close()
