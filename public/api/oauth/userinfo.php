...
if (strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET')) !== 'GET') {
    header('Allow: GET');
    insight_oauth_error('invalid_request', 'Use GET.', 405);
}
...
if ($identity === null) {
    ...
    insight_oauth_error('invalid_token', 'Invalid or expired token.', 401);
}
if (($identity['forbidden'] ?? false) === true) {
    insight_oauth_error('insufficient_scope', 'The openid scope is required.', 403);
}
