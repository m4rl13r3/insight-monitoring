...
$identity = insight_access_require_api([$method === 'GET' ? 'maintenances:read' : 'maintenances:write']);
if ($method === 'GET') {
    try {
    ...
    try {
        $result = match ($method) {
            'POST' => insight_maintenances_create($insightAdminConfig, $input, $apiUser),
            'DELETE' => insight_maintenances_delete($insightAdminConfig, $id),
            default => insight_maintenances_update($insightAdminConfig, $id, $input),
        };
    } catch (Throwable) {
        $result = ['ok' => false, 'status_code' => 503, 'error' => 'database_unavailable'];
    }
